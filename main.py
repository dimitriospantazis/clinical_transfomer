...
resume_run_id = "45cbcda4ae2c43f7adc19f271a36064f"
model_uri = f"runs:/{resume_run_id}/best_model"
best_model = mlflow.pytorch.load_model(model_uri)
best_model.eval().to("cpu")

# A. True sequence
token_id = val_data[19]
token_id = train_data[1]
true_seq = joint_tokenizer.decode(token_id)

# B. Predicted sequence

collator = Collator(joint_tokenizer, mlm=False, mlm_probability=0.0, mlm_target="both-linked", shuffle_tokens=False)
output = best_model(collator([token_id]))

# The model output is a dictionary. We assume the logits tuple is:
#   (prediction_head_logits, mlm_logits_n, mlm_logits_v, mlm_logits_v_num)
# Here, we extract the logits from the categorical head for names.
mlm_logits_n = output['logits'][1]  # shape: (batch_size, seq_length, vocab_size_n)
mlm_logits_v = output['logits'][2]  # shape: (batch_size, seq_length, vocab_size_n)
mlm_logits_v_num = output['logits'][3]  # shape: (batch_size, seq_length, vocab_size_n)

# Get predicted token IDs by taking the argmax along the vocab dimension.
predicted_token_ids_n = torch.argmax(mlm_logits_n, dim=-1).squeeze(0)  # shape: (batch_size, seq_length)
predicted_token_ids_v = torch.argmax(mlm_logits_v, dim=-1).squeeze(0).float()  # shape: (batch_size, seq_length)
predicted_token_ids_v_num = torch.tensor(mlm_logits_v_num.squeeze(0).T.squeeze(0))  # shape: (batch_size, seq_length)
predicted_token_ids_v = torch.where(predicted_token_ids_v_num != 0, predicted_token_ids_v_num, predicted_token_ids_v.float())

# Estimated token_id
token_id_est = {}
token_id_est['input_ids_n'] = predicted_token_ids_n
token_id_est['input_ids_v'] = predicted_token_ids_v

# Hack, use original type_mask_n and type_mask_v
token_id_est['type_mask_n'] = torch.tensor(token_id['type_mask_n'])
token_id_est['type_mask_v'] = torch.tensor(token_id['type_mask_v'])

est_seq = joint_tokenizer.decode(token_id_est)
